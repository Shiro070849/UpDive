<template>
  <div class="flex justify-center items-center min-h-screen bg-cover bg-center bg-no-repeat login-wrapper">
    <div class="bg-white/90 backdrop-blur-lg rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.1)] py-6 px-7 w-full max-w-[320px] text-center">
      ...
      <!-- Title -->
      <h1 class="text-base font-semibold text-gray-600 mb-4 tracking-normal">Sign in to UpDive</h1>

      <!-- Login Form -->
      <form class="flex flex-col gap-2 mb-2" @submit.prevent="handleLogin">
        <input
      ...
      <a href="#" class="inline-block mt-2 text-[#5B9BD5] text-[0.6875rem] no-underline opacity-75 transition-all duration-200 hover:text-[#4A8BC2] hover:opacity-100 hover:underline">Forgot Password?</a>

      <!-- Register Link -->
      <div class="mt-2 text-xs text-gray-500">
        Don't have an account?
        <router-link to="/register" class="text-[#5B9BD5] text-xs font-medium hover:text-[#4A8BC2] hover:underline transition-all duration-200 ml-1">Sign up</router-link>
      </div>
    </div>
  </div>
</template>
